...
class DataTabelController extends Controller {
                ...
                ->addColumn('action', function($user){
                    ...
                    if (Auth::user()->hasPermission('users.update')) {
                        $actions .= '<button type="button"
                                    ...
                                    class="permissions-user-btn block w-full text-right px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-white" role="menuitem">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="inline ml-2">
    ...
    public function institutions(Request $request) {
        if (!Auth::user()->hasPermission('institutions.view')) {
            if ($request->ajax()) {
                return DataTables::of([])->make(true);
            }
            return redirect()->route('assets');
    ...
    public function languages(Request $request) {
        if (!Auth::user()->hasPermission('languages.view')) {
            if ($request->ajax()) {
                return DataTables::of([])->make(true);
            }
            return redirect()->route('assets');
        ...
        if ($request->ajax()) {
            ...
            ->addColumn('actions', function ($word) {
                ...
                if (Auth::user()->hasPermission('languages.update')) {
                    $actions .= '<a href="javascript:void(0)" class="btn btn-icon btn-outline-primary" onclick="editLanguage(\'' . addslashes($word['word']) . '\')"><i class="mdi mdi-pencil"></i></a> ';
                }
                
                if (Auth::user()->hasPermission('languages.delete')) {
                    $actions .= '<a href="javascript:void(0)" class="btn btn-icon btn-outline-danger" onclick="deleteLanguage(\'' . addslashes($word['word']) . '\')"><i class="mdi mdi-trash-can"></i></a>';
                }
        ...
        if ($request->ajax()) {
            ...
            ->addColumn('actions', function ($permission) {
                ...
                if (Auth::user()->hasPermission('permissions.update')) {
                    $actions .= '<a href="javascript:void(0)" class="btn btn-icon btn-outline-primary" onclick="editPermission(' . $permission->id . ')"><i class="mdi mdi-pencil"></i></a> ';
                }
                
                if (Auth::user()->hasPermission('permissions.delete')) {
                    $actions .= '<a href="javascript:void(0)" class="btn btn-icon btn-outline-danger" onclick="deletePermission(' . $permission->id . ')"><i class="mdi mdi-trash-can"></i></a>';
                }
        ...
        if ($request->ajax()) {
            return DataTables::of($assets)
                ...
                ->addColumn('action', function($asset){
                    ...
                    if (Auth::user()->hasPermission('assets.view')) {
                        $actions .= '<a href="'.route('assets.show', $asset->id).'"
                    ...
                    if (Auth::user()->hasPermission('assets.update')) {
                        $actions .= '<a href="'.route('assets.edit', $asset->id).'"
                                    class="edit-asset-btn block w-full text-right px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-white" role="menuitem">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="inline ml-2">
                                        <path d="M7 7H6a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2-2v-1"></path>
                                        <path d="M20.385 6.585a2.1 2.1 0 0 0-2.97-2.97L9 12v3h3l8.385-8.415Z"></path>
    ...
    public function types(Request $request) {
        if (!Auth::user()->hasPermission('types.view')) {
            if ($request->ajax()) {
                return DataTables::of([])->make(true);
            }
            return redirect()->route('assets');
        ...
        if ($request->ajax()) {
            return DataTables::of($query)
                ...
                ->addColumn('action', function($type){
                    ...
                    if (Auth::user()->hasPermission('types.update')) {
                        $actions .= '<button type="button"
                                    data-id="'.$type->id.'"
                                    data-name="'.$type->name.'"
    ...
    public function depreciationEntries(Request $request) {
        if (!Auth::user()->hasPermission('depreciation-entries.view')) {
            if ($request->ajax()) {
                return DataTables::of([])->make(true);
            }
            return redirect()->route('assets');
        ...
        if ($request->ajax()) {
            return DataTables::of($entries)
                ...
                ->addColumn('action', function($entry){
                    ...
                    if (Auth::user()->hasPermission('depreciation-entries.update')) {
                    ...
                    if (Auth::user()->hasPermission('depreciation-entries.delete')) {
                        $actions .= '<div class="relative group inline-block">
                            <button type="button"
