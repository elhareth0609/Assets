...
@section('title', __('إهلاك الأصول الثابتة'))
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        @if (Auth::user()->hasPermission('depreciation-entries.view'))
            <div class="flex gap-2">
                ...
                <button id="import-depreciation-entries-btn" class="inline-flex items-center justify-center rounded-md font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-primary-400 focus:ring-offset-2 disabled:opacity-50 disabled:pointer-events-none bg-transparent border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:text-slate-100 dark:hover:bg-slate-800 h-8 px-3 text-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-download w-4 h-4 ltr:mr-2 rtl:ml-2">
                        <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                        <polyline points="7 10 12 15 17 10"></polyline>
                        <line x1="12" x2="12" y1="15" y2="3"></line>
                ...
                <a href="{{ route('depreciation-entries.import-template') }}" class="inline-flex items-center justify-center rounded-md font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-primary-400 focus:ring-offset-2 disabled:opacity-50 disabled:pointer-events-none bg-transparent border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:text-slate-100 dark:hover:bg-slate-800 h-8 px-3 text-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-download w-4 h-4 ltr:mr-2 rtl:ml-2">
                        <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                        <polyline points="7 10 12 15 17 10"></polyline>
                        <line x1="12" x2="12" y1="15" y2="3"></line>
                ...
                @if (Auth::user()->hasPermission('depreciation-entries.create'))
...
<div id="importDepreciationentriesModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl max-w-md w-full mx-4">
        <div class="p-6">
            <div class="flex justify-between items-center mb-4">
            ...
            <form id="importDepreciationentriesForm" action="{{ route('depreciation-entries.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
...
$(document).ready(function() {
    ...
    $('#importDepreciationentriesForm').on('submit', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                $('#importDepreciationentriesModal').addClass('hidden').removeClass('flex');
    ...
    // Import Employee Modal
    $('#import-depreciation-entries-btn').on('click', function() {
        $('#importDepreciationentriesModal').removeClass('hidden').addClass('flex');
        $('#importDepreciationentriesForm')[0].reset();
        $('#file-name').text('');
    });
