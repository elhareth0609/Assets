    ...
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl max-w-md w-full mx-4">
        <div class="p-6">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-medium text-slate-900 dark:text-white">إضافة نوع جديد</h3>
...
<div id="editTypeModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl max-w-md w-full mx-4">
        <div class="p-6">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-medium text-slate-900 dark:text-white">تعديل النوع</h3>
...
<div id="deleteTypeModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl max-w-md w-full mx-4">
        <div class="p-6">
            ...
            <p class="text-center text-slate-500 dark:text-slate-400 mb-6">هل أنت متأكد من حذف النوع: <span id="deleteTypeName" class="font-semibold"></span>؟ لا يمكن التراجع عن هذا الإجراء.</p>
            <div class="flex justify-center flex-row-reverse space-x-4">
...
<style>
  ...
  html.dark #types-table tbody tr {
    border-bottom: 1px solid rgb(51 65 85);
  }
  #types-table tbody tr:hover {
    background-color: rgb(248 250 252);
  }
  html.dark #types-table tbody tr:hover {
  ...
  html.dark #types-table tbody td {
  ...
  html.ltr #types-table thead th {
    text-align: left;
  }
  html.rtl #types-table thead th {
    text-align: right;
  }
...
$(document).ready(function() {
    ...
    function applyTailwindStyles() {
        ...
        $("#types-table").addClass("w-full caption-bottom text-sm border-collapse");
        $("#types-table thead").addClass("bg-slate-50 dark:bg-slate-800");
        $("#types-table thead tr").addClass("border-b border-slate-200 dark:border-slate-700 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50");
        $("#types-table tbody tr").addClass("border-b border-slate-200 dark:border-slate-700 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50");
    ...
    $('#createTypeForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                $('#createTypeModal').addClass('hidden').removeClass('flex');
                showNotification('تم إضافة النوع بنجاح', 'success');
                table.ajax.reload();
            },
            error: function(xhr) {
    ...
    $('#editTypeForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                $('#editTypeModal').addClass('hidden').removeClass('flex');
                showNotification('تم تحديث النوع بنجاح', 'success');
                table.ajax.reload();
            },
            error: function(xhr) {
                var errors = xhr.responseJSON.errors;
    ...
    $(document).on('click', '.delete-type-btn', function() {
        ...
        $('#confirmDeleteTypeBtn').off('click').on('click', function() {
            $.ajax({
                ...
                error: function(xhr) {
                    $('#deleteTypeModal').addClass('hidden').removeClass('flex');
                    showNotification('حدث خطأ أثناء حذف النوع', 'error');
                }
            });
        });
    });
